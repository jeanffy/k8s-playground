...
- `redis` : le serveur Redis en tant que base de données
...
- les infos de l'api sont :
  ...
  - la liste des clés/valeurs du serveur Redis associé (si la variable d'environnement est spécifiée - voir ci-après)

A chaque visite sur la page, un nouveau fichier est créé (et une nouveau couple clé/valeur est ajouté à Redis si utilisé).
...
Si Redis est utilisé, l'application se connecte par défaut sur `127.0.0.1:6379`. Il est possible de modifier :

- le host avec `NODUL_REDIS_HOST`
- le port avec `NODUL_REDIS_PORT`
...
> Dans ce cas, le serveur Redis n'est pas utilisé.
...
> Dans ce cas, le serveur Redis pas utilisé.
...
Le dossier temporaire utilisé par l'application est monté dans `data/file` afin de persister les fichiers créés. Le dossier de données de Redis est quant à lui monté dans `data/redis`.
...
- `sh deploy/docker/publish-images-no-push.sh`
...
- Stopper l'application : `docker compose --project-directory deploy/docker-compose down` (le containers sont supprimés)
...
- `kubectl logs -f nodulapi-pod redis` pour voir les logs de Redis
...
- `kubectl get all -l app=nodulapp` pour voir tous les objets créés
- si minikube avec driver Docker
...
- `kubectl get pods` pour voir les pods créés
- `kubectl get pvc` pour voir les volumes créés
- `kubectl get ingresses` pour voir le service ingress créé
- `kubectl logs -f nodulapi-deployment-xxx-yyy` pour voir les logs de l'app Node.js API
- `kubectl logs -f redis-deployment-xxx-yyy` pour voir les logs de Redis
...
- si minikube avec driver Docker
  - `minikube service nodulapi-clusterip` (spécifique minikube)
  - Les fichiers et les valeurs Redis sont toujours présents
- sinon
  - `minikube ip`
  - http://<minikube-ip>
...
- ou
  ...
  - `kubectl delete pvc -l app=nodulapp` (les fichiers crées et les données de Redis seront perdues)
...
- faire un front en plus de l'api et inclure ça dans ingress (autre path)
- ajouter un namespace
- load balancing
- mise à jour docker hub image
- utiliser des secrets (login/pwd pour accéder à redis)
